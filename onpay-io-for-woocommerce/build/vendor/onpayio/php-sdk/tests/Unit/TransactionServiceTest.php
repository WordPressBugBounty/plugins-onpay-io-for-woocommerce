...
class TransactionServiceTest extends TestCase
{
    ...
    public function testGetTransactionReturnsDetailedTransactionOnValidIdentifier()
    {
        ...
        $this->apiMock->method('get')->with('transaction/' . $uuid)->willReturn($mockResult);
    ...
    public function testCaptureTransactionReturnsDetailedTransactionOnValidNumber()
    {
        $uuid = '123e4567-e89b-12d3-a456-426614174000';
        $mockResult = ['data' => ['uuid' => $uuid], 'links' => ['self' => '/transaction/' . $uuid]];
        $this->apiMock->method('post')->with('transaction/' . $uuid . '/capture', $this->anything())->willReturn($mockResult);
        $service = new TransactionService($this->apiMock);
