...
use WoocommerceOnpay\OnPay\OAuth\Client\Http\HttpClientInterface;
use WoocommerceOnpay\OnPay\OAuth\Client\Session;
use WoocommerceOnpay\OnPay\OAuth\Client\SessionInterface;
use WoocommerceOnpay\OnPay\OAuth\Client\Exception\AuthorizeException;
use WoocommerceOnpay\OnPay\OAuth\Client\Exception\OAuthException;
use WoocommerceOnpay\OnPay\OAuth\Client\Exception\TokenException;
use WoocommerceOnpay\OnPay\OAuth\Client\Http\Request;
use WoocommerceOnpay\OnPay\InternalTokenStorage;
...
class OAuthClient
{
    ...
    public function getAuthorizeUri(Provider $provider, $userId, $scope, $redirectUri)
    {
        ...
        $queryParameters = ['client_id' => $provider->getClientId(), 'redirect_uri' => $redirectUri, 'scope' => $scope, 'state' => \str_replace('=', '', \base64_encode(\random_bytes(32))), 'response_type' => 'code', 'code_challenge_method' => 'S256', 'code_challenge' => \str_replace('=', '', \base64_encode(\hash('sha256', $codeVerifier, \true)))];
